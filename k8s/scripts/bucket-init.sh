...
until curl -s "$MINIO_ENDPOINT/minio/health/live" >/dev/null 2>&1; do
    ...
    if [ $retries -ge $max_retries ]; then
        echo "==> ERROR: MinIO did not start within $max_retries attempts"
        exit 1
    fi
    sleep 2
done
...
mc alias set "$MINIO_ALIAS" "$MINIO_ENDPOINT" "$MINIO_HOST" "$MINIO_PASS"
...
mc version enable "${MINIO_ALIAS}/${PRIVATE_BUCKET}"
...
mc version enable "${MINIO_ALIAS}/${PUBLIC_BUCKET}"
...
echo "$CORS_CONFIG" | mc cors set "${MINIO_ALIAS}/${PUBLIC_BUCKET}" /dev/stdin
